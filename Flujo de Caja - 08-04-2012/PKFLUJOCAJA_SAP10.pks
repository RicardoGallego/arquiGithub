...
/***********************************************************************************************
  ...
  Autor       : JR Gallego
  Fecha       : 24-03-2013
  Descripcion : Se modifico para que se retornara los meses en espanol
***********************************************************************************************/
    mesle         VARCHAR2(3):= 'DIC';
...
/***********************************************************************************************
 ...
 Desc   : Se adiciona un parametro mas donde se guarda la informacion del dia
          Se adiciono la excepcion al procedimiento 
***********************************************************************************************/
    sbError    VARCHAR2(2000) := NULL;

BEGIN
    INSERT INTO factconc_sap(facodepa,facoloca,facoanoe,facomese,facoano,facomes,
                             facoconc,facovlor,facogrup,facofunc, facodia)
                      VALUES(nudepa,nuloca,nuanoe,numese,nuano,numes,nuconc,nuValor,
                             nugrupo,nufunc,dtDia);
EXCEPTION
    WHEN OTHERS THEN
      sbError := '[PKFLUJOCAJA_SAP.PROCREAREGFACTCONC] - ' || SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
      pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
...
BEGIN
        ...
        ELSE
           ...
           EXCEPTION
               ...
               WHEN too_many_rows THEN
                   ...
                   --<<
                   -- No se debe guardar ninguna informacion en la bd
                   -->>
                   ROLLBACK;
           END;
        END IF;
       ...
       ELSE
           ...
           EXCEPTION
               ...
               WHEN too_many_rows THEN
                   ...
                   --<<
                   -- Si se produce un error y no puedo terminar todo el proceso, que no guarde 
                   -- ningun tipo de informacion en la bd
                   -->>
                   ROLLBACK;
           END;
        END IF;
  ...
  -- Generamos valor facturado mes2
    ...
    --<<
    -- Se Realiza un commit para guardar la informacion cuando todo ha salido correctamente
    -->>
    COMMIT;
...
EXCEPTION
    WHEN OTHERS THEN
         ...
         sbError := '[PKFLUJOCAJA_SAP.PROPROYINGFACTFLUJOCAJA] - ' || SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
         pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
         --<<
         -- Si se produce un error y no puedo terminar todo el proceso, que no guarde 
         -- ningun tipo de informacion en la bd
...
/***********************************************************************************************
   ...
   Autor  : JRG  
   Fecha  : abril 11 2013
   Desc   : Se adiona el commit y el roolback y se modifico el msn de la excepcion         
***********************************************************************************************/
    meseval         NUMBER;
    anoeval         NUMBER;
    nuano1          NUMBER(4);
    ...
    FOR i IN 1..to_char(last_day(dtfechdia), 'DD') LOOP 
        ...
        INSERT INTO compreca_sap(
                           ...
                           VALUES(
                                 ...
                                 ano2,sumvafa,sumrec1,sumrec2,nufunc,dtfechdia
                                );
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        sbError := '[PKFLUJOCAJA_SAP.PROYPAGOSFLUCAJA] - ' || SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
        pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
        --<<
        -- Si se produce un error y no puedo terminar todo el proceso, que no guarde 
        -- ningun tipo de informacion en la bd
...
BEGIN
    ...
    FOR i IN 1..to_char(last_day(dtFechInic1), 'DD') LOOP 
      ...
      SELECT nvl(sum(nvl(MVCBVLOR,0)),0) INTO recames1dia
        ...
        --porcdiames1:=round( (recames1dia/recames1)*100,0);
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        sbError := '[PKFLUJOCAJA_SAP.PROPROYRECAFLUJOCAJA] - ' || SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
        pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
        --<<
        -- Si se produce un error y no puedo terminar todo el proceso, que no guarde 
        -- ningun tipo de informacion en la bd
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        sbError := '[PKFLUJOCAJA_SAP.PROCONSVALREALINGCARG] - ' || SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
        pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
        --<<
        -- Si se produce un error y no puedo terminar todo el proceso, que no guarde 
        -- ningun tipo de informacion en la bd
...
PROCEDURE procarginreal(nuanoe NUMBER,numese NUMBER, dtdiae DATE) IS
 /************************************************************************
  ...
  Autor       : JR Gallego   
  Fecha       : abril 11 2013  
  Descripcion : Se modifico el msn de error para que fuera mas facil 
                encontrar una excepcion cuando ocurra
 *************************************************************************/
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        sbError := '[PKFLUJOCAJA_SAP.PROCARGINREAL] - ' || SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
        pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
...
EXCEPTION
    WHEN DUP_VAL_ON_INDEX THEN
        NULL;
    WHEN OTHERS THEN
        sbError := '[PKFLUJOCAJA_SAP.PROCONSVALREALINGTIPMOV] - ' || a1||' Loc. : '||b||' Grupo : '||c1||'Tip. Mov : '||d||' Cta. banc : '||e||'-'||f||' '||SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
        pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
...
--    SELECT last_day(dtfechinic) INTO dtfechfini FROM dual;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            sbError := '[PKFLUJOCAJA_SAP.PROGENINGREFLCJ] - ' || 'Error al consultar el ano y mes anterior al evaluado en pkflujocaja_sap.progeningreflcj. '||SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
            pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
            ROLLBACK;
    END;
    ...
    FETCH curecaprimMes INTO facturado2,recaudado2,nuano1,numes1;
        IF curecaprimMes%NOTFOUND THEN
            ...
            sbError := '[PKFLUJOCAJA_SAP.PROGENINGREFLCJ] - ' || 'Se requiere lo recaudado del primer mes para el periodo evaluado. '||to_char(nuanoeval)||' - '||to_char(numeseval)||' pkflujocaja_sap.progeningreflcj. '||SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
            pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
            ROLLBACK;
        END IF;
    CLOSE curecaprimMes;
 ...
 -- Obtenemos lo recaudado del segundo mes en el mes y a?o anterior al evaluado
     ...
     FETCH curecaSegMes INTO facturado,recaudado,nuano2,numes2;
         IF curecaSegMes%NOTFOUND THEN
             ...
             sbError := '[PKFLUJOCAJA_SAP.PROGENINGREFLCJ] - ' || 'Se requiere lo recaudado del segundo mes para el periodo evaluado. '||to_char(nuanoeval)||' - '||to_char(numeseval)||' pkflujocaja_sap.progeningreflcj. '||SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
             pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
             ROLLBACK;
         END IF;
     CLOSE curecaSegMes;
             ...
             FOR contador IN 1..to_char(last_day(dtfechinic), 'DD') LOOP 
                 ...
                 BEGIN
                  ...
                  EXCEPTION
                      WHEN OTHERS THEN
                          RAISE_APPLICATION_ERROR(-20013,'Error al consultar el porcentaje de recaudo por semana del primer mes en pkflujocaja_sap.progeningreflcj. '||SQLERRM);
                          sbError := '[PKFLUJOCAJA_SAP.PROGENINGREFLCJ] - ' || 'Error al consultar el porcentaje de recaudo por semana del primer mes en pkflujocaja_sap.progeningreflcj. '||SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
                          pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
                          ROLLBACK;
                  END;    
...
/*                         IF nuCountTemp < 20 THEN
                 ...
                 EXCEPTION
                     WHEN OTHERS THEN
                         RAISE_APPLICATION_ERROR(-20014,'Error al insertar en tereflcj_sap desde pkflujocaja_sap.progeningreflcj. Agencia : '||j.locadepa||'-'||j.locacodi||' grupo '||nugrupo||' concepto '||i.concepto||' '||SQLERRM);
                         sbError := '[PKFLUJOCAJA_SAP.PROGENINGREFLCJ] - ' || 'Error al insertar en tereflcj_sap desde pkflujocaja_sap.progeningreflcj. Agencia : '||j.locadepa||'-'||j.locacodi||' grupo '||nugrupo||' concepto '||i.concepto||' '||SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
                         pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
                         ROLLBACK;
                 END;
        ...
        FOR i IN (SELECT trfcdepa depa,trfcloca loca,trfcgrup grup,
            ...
            EXCEPTION
                ...
                WHEN OTHERS THEN
                    ...
                    sbError := '[PKFLUJOCAJA_SAP.PROGENINGREFLCJ] - ' || '1 SUM1'||i.grup||' '||SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
                    pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
                    ROLLBACK;
            END;
        ...
        FOR i IN (SELECT trfcgrup grup,SUM(trfcdiapm) diap, SUM(trfcrvca) val_ca, SUM(trfcrvtm) tm 
            ...
            EXCEPTION
                ...
                WHEN OTHERS THEN
                    ...
                    sbError := '[PKFLUJOCAJA_SAP.PROGENINGREFLCJ] - ' || '2 SUM1'||i.grup||' '||SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
                    pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
                    ROLLBACK;
            END;
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        sbError := '[PKFLUJOCAJA_SAP.PROGENINGREFLCJ] - ' || SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
        pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
        --<<
        -- Si se produce un error y no puedo terminar todo el proceso, que no guarde 
        -- ningun tipo de informacion en la bd
...
PROCEDURE proLlenFlCj IS
/***********************************************************************************************
  Nombre : proEnviFlCj
  Desc   : Llena la tabla ldc_flujoCaja con los datos adecuados 
  Autor  : Jose Gallego
  Fecha  : 10-abril-2013
...
BEGIN
    ...
    DELETE LDC_FLUJOCAJA WHERE ano = anoeval AND mes = meseval;
    
    INSERT INTO LDC_FLUJOCAJA
      (ano, mes, cdconcep, tipoflu, valor, moneda, fecha_doc)
      SELECT TRFCANOE ANO, TRFCMESE MES, TFCONFLCJ cdconcep, 1 TIPOFLU,
               round (SUM(TRFCDIAPM + TRFCRVCA)) VALOR,
               'COP' MONEDA, to_date(to_char(trfcdiae,'DD/MM/YYYY'), 'DD/MM/YYYY') FECHA_DOC
        ...
        GROUP BY TRFCANOE, TRFCMESE, TFCONFLCJ, TRFCFLTR, to_date(to_char(trfcdiae,'DD/MM/YYYY'), 'DD/MM/YYYY')
        HAVING SUM(TRFCDIAPM + TRFCRVCA) > 0;
    
     COMMIT;   
    
EXCEPTION
    WHEN OTHERS THEN
        sbError := '[PKFLUJOCAJA_SAP.PROLLENFLCJ] - ' || SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
        pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
        ROLLBACK;  
...
/***********************************************************************************************
  Nombre : proReportes
  Desc   : Genera el reporte del valor real de un mes determinado y el valor proyectado
           del siguiente mes 
  Autor  : Jose Gallego
  Fecha  : 10-abril-2013
...
EXCEPTION
    WHEN OTHERS THEN
        sbError := '[PKFLUJOCAJA_SAP.PROREPORTES] - ' || SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
        pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
        ROLLBACK;  
...
/***********************************************************************************************
  Nombre : proReportes
  Desc   : Esta procedimiento genera todo el proceso de ingresos del flujo de caja y lleva 
           las tramas de cuales procedimientos no se puedieron realizar 
  Autor  : Jose Gallego
  Fecha  : 10-abril-2013
  
Parametros de Entrada
   nuAno       NUMBER     -> Ano a evaluar
   nuMes       NUMBER     -> Mes a evaluar
   nuOperacion NUMBER     -> nuemero de las operaciones que se desean realizar
                             1- ProProyIngFactFlujoCaja
                             2- proypagosflucaja
                             3- ProProyRecaFlujoCaja
                             4- proconsvalrealingcarg
                             5- proconsvalrealingtipmov
Parametros de Salida
...
***********************************************************************************************/
    ...
    -- Variables
    -->>
    sbOperReal       VARCHAR2(2000):='0';  -- Operacion Realizada
    sbError          VARCHAR2(2000) := NULL; -- Capturar Error
    nuOper           NUMBER:=0;
BEGIN
    nuOper := nuOperacion; 
    pkflujocaja_sap10.proInicial(nuanoe, numese);
    
    --pkregidepu.pRegiMensaje('trazabilidad ', nuOper, 'PKFLUJOCAJA_SAP');
    
    IF nuOper < 1 THEN
        pkflujocaja_sap10.ProProyIngFactFlujoCaja;
        sbOperReal := '1. PRCGEINCOMP-PROPROYINGFACTFLUJOCAJA';
        pkregidepu.pRegiMensaje('EXITO OPERACION ', sbOperReal, 'PKFLUJOCAJA_SAP');
    END IF;
   
    IF nuOper < 2 THEN 
        pkflujocaja_sap10.proypagosflucaja;
        sbOperReal := '2. PRCGEINCOMP-PROYPAGOSFLUCAJA';
        pkregidepu.pRegiMensaje('EXITO OPERACION ', sbOperReal, 'PKFLUJOCAJA_SAP');
    END IF;
    
    IF nuOper < 3 THEN 
        pkflujocaja_sap10.ProProyRecaFlujoCaja;
        sbOperReal := '3. PRCGEINCOMP-PROPROYRECAFLUJOCAJA';
        pkregidepu.pRegiMensaje('EXITO OPERACION ', sbOperReal, 'PKFLUJOCAJA_SAP');
    END IF;
    
    IF nuOper < 4 THEN
        pkflujocaja_sap10.proconsvalrealingcarg;
        sbOperReal := '4. PRCGEINCOMP-PROCONSVALREALINGCARG';
        pkregidepu.pRegiMensaje('EXITO OPERACION ', sbOperReal, 'PKFLUJOCAJA_SAP');
    END IF;
    
    IF nuOper < 5  THEN
        pkflujocaja_sap10.proconsvalrealingtipmov;
        sbOperReal := '5. PRCGEINCOMP-PROCONSVALREALINGTIPMOV';
        pkregidepu.pRegiMensaje('EXITO OPERACION ', sbOperReal, 'PKFLUJOCAJA_SAP');
    END IF;

    IF nuOper < 6  THEN
        pkflujocaja_sap10.progeningreflcj;
        sbOperReal := '6. PRCGEINCOMP-PROGENINGREFLCJ';
        pkregidepu.pRegiMensaje('EXITO OPERACION ', sbOperReal, 'PKFLUJOCAJA_SAP');
    END IF;
    
    IF nuOper < 7  THEN
        pkflujocaja_sap10.proLlenFlCj;
        sbOperReal := '7. PRCGEINCOMP-PROLLENFLCJ';
        pkregidepu.pRegiMensaje('EXITO OPERACION ', sbOperReal, 'PKFLUJOCAJA_SAP');
    END IF;
    
EXCEPTION
    WHEN OTHERS THEN
        sbError := '[PKFLUJOCAJA_SAP.PRCGEINCOMP] - ' || sbOperReal|| ' - '|| SQLERRM || ' ' || DBMS_UTILITY.format_error_backtrace;
        pkregidepu.pRegiMensaje('EXCEPTION Error', sbError, 'PKFLUJOCAJA_SAP');
        ROLLBACK; 
END prcGeInComp;
...
BEGIN
    ...
    --sbError := nuanoe || ' - ' || numese || ' Prueba Form';
    pkregidepu.pRegiMensaje('EXCEPTION Error ', nuanoe || ' - ' || numese || ' Prueba Form', 'PKFLUJOCAJA_SAP');
