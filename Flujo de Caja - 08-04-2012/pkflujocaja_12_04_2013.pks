CREATE OR REPLACE PACKAGE FINAN.pkflujocaja IS
...
END;
/
CREATE OR REPLACE PACKAGE BODY FINAN.pkflujocaja IS
...
PROCEDURE procrearegfactconc(nudepa NUMBER,nuloca NUMBER,nuanoe NUMBER,numese NUMBER,nuano NUMBER,numes NUMBER,nuconc NUMBER,nuValor NUMBER,nugrupo NUMBER) IS
/***********************************************************************************************
 Nombre : procrearegfactconc
 Desc   : Crea registro en factconc
 Autor  : John Jimenez
 Fecha  : Junio 1 2007
***********************************************************************************************/
BEGIN
  INSERT INTO factconc(
                       facodepa,facoloca,facoanoe,facomese,facoano,facomes,facoconc,facovlor,facogrup,facofunc
                      )
               VALUES(
                       nudepa,nuloca,nuanoe,numese,nuano,numes,nuconc,nuValor,nugrupo,nufunc
                      );
...
CURSOR cuCargos(dtFechI DATE,dtFechF DATE) IS
    SELECT /*+ index (cargos idxcargos11)*/ locadist cargdepa,locaagen cargloca,cargconc,
     ...
     WHERE cargfecr >= dtFechI
      ...
      GROUP BY locadist,locaagen,cargconc,cargcate
    UNION ALL
    SELECT /*+ index (cargos idxcargos11)*/  locadist cargdepa,locaagen cargloca,cargconc,
	   sum(decode(cargsign,'DB',nvl(cargvalo,0),nvl(-cargvalo,0))) SUMA,cargcate cate
...
BEGIN
  ...
  DELETE factconc WHERE facoanoe = anoeval AND facomese = meseval AND facofunc = nufunc;
  DELETE peevflcj WHERE pefcanoe = anoeval AND pefcmese = meseval AND pefctire = 'F' AND func = nufunc;
  ...
  FOR i IN cucargos(dtfechinic,dtfechfini) LOOP
    ...
    END IF;
    IF nugrupo <> -1 THEN
     procrearegfactconc(i.cargdepa,i.cargloca,anoeval,meseval,ano,mes,i.cargconc,i.suma,nugrupo);
    END IF;
  END LOOP;
  ...
  FOR i IN cucargos(dtfechinic2,dtfechfini2) LOOP
   ...
   IF i.cargconc = 25 AND i.cate = 3 THEN
    ...
    END IF;
    IF nugrupo <> -1 THEN
     procrearegfactconc(i.cargdepa,i.cargloca,anoeval,meseval,ano2,mes2,i.cargconc,i.suma,nugrupo);
    END IF;
  END LOOP;
  ...
  INSERT INTO peevflcj VALUES(anoeval,meseval,'F',nufunc);

EXCEPTION
 WHEN OTHERS THEN
  RAISE_APPLICATION_ERROR(-20005,'Error en pkflujocaja.ProProyIngFactFlujoCaja. '||SQLERRM);
...
mes1            NUMBER;
mes2            NUMBER;
sumvafa         compreca.corefact%type DEFAULT 0;
sumrec1         compreca.corereca%type DEFAULT 0;
sumrec2         compreca.corerec2%type DEFAULT 0;
n               NUMBER;
...
BEGIN
  ...
  DELETE compreca WHERE coreanoe = anoeval AND coremese = meseval AND corefunc = nufunc;
  DELETE peevflcj WHERE pefcanoe = anoeval AND pefcmese = meseval AND pefctire = 'P' AND func = nufunc;
...
--     ano1:=anoeval;
  ...
  -- Calculamos el facturado del mes evaluado
    FOR c IN cuco(nuano1,meseval) LOOP
     sumvafa := nvl(sumvafa,0) + nvl(c.cucovato,0);

  -- Calculamos lo recaudado 1 y 2 meses despues
    FOR a IN carg(c.cucocodi) LOOP
       n:=(a.cargano*12+a.cargmes-1)-(c.cucoano*12+c.cucomes);
       IF n=0 THEN
          sumrec1 := nvl(sumrec1,0) + nvl(a.cargvalo,0);
       ELSE
         sumrec2 := nvl(sumrec2,0) + nvl(a.cargvalo,0);
       END IF;
    END LOOP;
   END LOOP;

        INSERT INTO compreca(
    ...
    INSERT INTO peevflcj VALUES(anoeval,meseval,'P',nufunc);

EXCEPTION
 WHEN OTHERS THEN
  RAISE_APPLICATION_ERROR(-20006,'Error en pkflujocaja.proypagosflucaja. '||SQLERRM);
...
dtFechF2       DATE;
recames1       movicuba.MVCBVLOR%type;
recames2       movicuba.MVCBVLOR%type;
sem1inicmes1   DATE;
sem2inicmes1   DATE;
sem3inicmes1   DATE;
sem4inicmes1   DATE;
...
sem3finmes1    DATE;
sem4finmes1    DATE;
recames1sem1   movicuba.MVCBVLOR%type;
recames1sem2   movicuba.MVCBVLOR%type;
recames1sem3   movicuba.MVCBVLOR%type;
recames1sem4   movicuba.MVCBVLOR%type;
porcsem1mes1   recaproy.reprs1m1%type;
porcsem2mes1   recaproy.reprs2m1%type;
porcsem3mes1   recaproy.reprs3m1%type;
porcsem4mes1   recaproy.reprs4m1%type;

BEGIN
  anoeval   := nuanorfc;
  meseval   := numesrfc;

  DELETE recaproy WHERE repranoe = anoeval ANd reprmese = meseval AND reprfunc = nufunc;
  DELETE peevflcj WHERE pefcanoe = anoeval AND pefcmese = meseval AND pefctire = 'R' AND func = nufunc;
  ...
  dtFechF1 := to_date(periodo1fini,'DD/MON/YYYY');
  ...
  --averiguar recaudo por semana en mes1
    ...
    sem4finmes1 := to_date(dtFechF1,'DD/MON/YYYY');
    ...
    INSERT INTO peevflcj VALUES(anoeval,meseval,'R',nufunc);

EXCEPTION
WHEN OTHERS THEN
 RAISE_APPLICATION_ERROR(-20007,'Error en pkflujocaja.ProProyRecaFlujoCaja. '||SQLERRM);
...
*************************************************************************/
 ...
 numes NUMBER;
BEGIN
   DELETE inreflcj WHERE ano = nuanorfc AND mes = numesrfc AND func = nufunc;
   ...
   SELECT last_day(dtfechaini) INTO dtfechafin
     FROM dual;
   dtfechafin  := to_date(dtfechafin,'DD/MON/YYYY');
    ...
    END IF;
     IF nugrupo <> -1 THEN
       INSERT INTO inreflcj(depa,loca,ano,mes,grupo,concep,valor,func)
...
PROCEDURE procarginreal(nuanoe NUMBER,numese NUMBER) IS
 /************************************************************************
  ...
  AUTOR          : John Jairo Jimenez Marimon
  FECHA          : Mayo 26 del 2007
  DESCRIPCION    : Llena la table tereflcj con los ingresos reales
 ...
 *************************************************************************/
  ...
  CURSOR cuCargreal IS
    SELECT depa,loca,ano,mes,grupo,concep,nvl(SUM(valor),0) valor FROM inreflcj WHERE ano = nuanoe AND mes = numese AND func = nufunc group by depa,loca,ano,mes,grupo,concep;
 --  UNION ALL
 --   SELECT depa,loca,ano,mes,grupo,concep,nvl(SUM(valor),0) valor FROM inreflcj@LIN_SUCRE WHERE ano = nuanoe AND mes = numese AND func = nufunc group by depa,loca,ano,mes,grupo,concep
 --  UNION ALL
 --   SELECT depa,loca,ano,mes,grupo,concep,nvl(SUM(valor),0) valor FROM inreflcj@LIN_CORDOBA WHERE ano = nuanoe AND mes = numese AND func = nufunc group by depa,loca,ano,mes,grupo,concep;
  ...
  FOR i IN cuCargreal LOOP
   UPDATE tereflcj
    ...
    IF sql%notfound THEN
     INSERT INTO tereflcj(trfcdepa,trfcloca,trfcanoe,trfcmese,trfcgrup,trfcconc,trfcfltr,trfcrvca,trfctipo,trfcfunc)
 ...
 *************************************************************************/
  ...
  CURSOR cuTipMovReal(dtfechainiP DATE,dtfechafinP DATE) IS
   SELECT mvcbdage depa,
     ...
     FROM movicuba,flcjtipmov,tipmovflcja
  ...
  GROUP BY mvcbdage,mvcbagen,fctmcdgo,mvcbtimv,fctmineg,mvcbdist,mvcbcons,mvcbdisa,mvcbtmad,mvcbcasi;
   ...
   /*SELECT mvcbesta esta,mvcbfeapl fecha,locadepa depa,locacodi loca,fctmcdgo grupo,mvcbtimv tipo_movim,FCTMINEG ineg,MVCBDIST,MVCBCONS,MVCBDISA,MVCBTMAD,MVCBCASI,nvl(SUM(mvcbvlor),0) nuvalor
     FROM movicuba,localida,flcjtipmov,tipmovflcja
    WHERE mvcbfeapl BETWEEN dtfechainip AND dtfechafinp
 ...
 f          NUMBER;
BEGIN
  DELETE ctabflcj WHERE ano = nuanorfc AND mes = numesrfc AND func = nufunc;
 ...
 FOR i IN cuTipMovReal(dtfechaini,dtfechafin) LOOP
   INSERT INTO ctabflcj(DEPA,LOCA,ano,mes,GRUPO,TIPMOV,DISTC,CONSC,DISA,TMAD,CASI,VALOR,tipo,func)
 ...
 EXCEPTION
 	WHEN DUP_VAL_ON_INDEX THEN
 	     NULL;
  WHEN OTHERS THEN
  RAISE_APPLICATION_ERROR(-20009,'Error al insertar en CTABFLCJ. Dpto : '||a||' Loc. : '||b||' Grupo : '||c||'Tip. Mov : '||d||' Cta. banc : '||e||'-'||f||' '||SQLERRM);
...
PROCEDURE procarginrealtipmov(nuanoe NUMBER,numese NUMBER) IS
 /************************************************************************
  ...
  AUTOR          : John Jairo Jimenez Marimon
  FECHA          : Mayo 26 del 2007
  DESCRIPCION    : Llena la table tereflcj con los ingresos reales de cta bancarias
 ...
 BEGIN
   FOR i IN cuCargreal LOOP
    ...
    WHERE trfcdepa = i.depa
      ...
      AND trfcfunc = nufunc;
    IF sql%notfound THEN
     INSERT INTO tereflcj(trfcdepa,trfcloca,trfcanoe,trfcmese,trfcgrup,trfcconc,trfcfltr,trfcrvtm,trfctipo,trfcfunc)
...
************************************************************************/
 nuanoeval        NUMBER(4);
 numeseval        NUMBER(2);
 facturado        compreca.corefact%TYPE;
 facturado2       compreca.corefact%TYPE;
 recaudado        compreca.corereca%TYPE;
 recaudado2       compreca.corereca%TYPE;
 nuanoant         NUMBER(4);
 numesant         NUMBER(2);
 ...
 nugrupo          tiflconc.tfconflcj%TYPE;
...
--  Consultamos lo recaudado del primer mes en el mes y a?o anterior al evaluado
  ...
  CURSOR curecaprimMes(anoante NUMBER,mesante NUMBER) IS
   ...
   /*       UNION ALL
           SELECT nvl(corefact,0) fact,nvl(corerec2,0) rec2,coreano ano,coremes mes
             FROM compreca@LIN_SUCRE
            WHERE coreano2 = anoante
              AND coremes2 = mesante
              AND corefunc = nufunc
          UNION ALL
           SELECT nvl(corefact,0) fact,nvl(corerec2,0) rec2,coreano ano,coremes mes
             FROM compreca@LIN_CORDOBA
            WHERE coreano2 = anoante
              AND coremes2 = mesante
              AND corefunc = nufunc  */
              ) X
 ...
 -- Se borran los registros para el a?o y mes a evaluar
    DELETE tereflcj WHERE trfcanoe = nuanoeval AND trfcmese = numeseval AND trfcfunc = nufunc;
 ...
 -- Consulta a?o y mes anterior al evaluado
   ...
   EXCEPTION
    WHEN OTHERS THEN
     RAISE_APPLICATION_ERROR(-20010,'Error al consultar el a?o y mes anterior al evaluado en pkflujocaja.progeningreflcj. '||SQLERRM);
   END;
 ...
 -- Obtenemos lo recaudado del primer mes en el mes y a?o anterior al evaluado
    ...
    FETCH curecaprimMes INTO facturado2,recaudado2,nuano1,numes1;
       IF curecaprimMes%NOTFOUND THEN
          CLOSE curecaprimMes;
          RAISE_APPLICATION_ERROR(-20011,'Se requiere lo recaudado del primer mes para el periodo evaluado. '||to_char(nuanoeval)||' - '||to_char(numeseval)||' pkflujocaja.progeningreflcj. '||SQLERRM);
       END IF;
    CLOSE curecaprimMes;
 ...
 -- Obtenemos lo recaudado del segundo mes en el mes y a?o anterior al evaluado
    ...
    FETCH curecaSegMes INTO facturado,recaudado,nuano2,numes2;
       IF curecaSegMes%NOTFOUND THEN
          CLOSE curecaSegMes;
          RAISE_APPLICATION_ERROR(-20012,'Se requiere lo recaudado del segundo mes para el periodo evaluado. '||to_char(nuanoeval)||' - '||to_char(numeseval)||' pkflujocaja.progeningreflcj. '||SQLERRM);
       END IF;
    CLOSE curecaSegMes;
 ...
 BEGIN
   ...
   EXCEPTION
    WHEN OTHERS THEN
      RAISE_APPLICATION_ERROR(-20013,'Error al consultar el porcentaje de recaudo por semana del primer mes en pkflujocaja.progeningreflcj. '||SQLERRM);
   END;
 ...
 -- Generamos los ingresos proyectados por semana
  ...
  -- Se graban los datos en la tabla
   ...
   EXCEPTION
    WHEN OTHERS THEN
    RAISE_APPLICATION_ERROR(-20014,'1Error al insertar en tereflcj desde pkflujocaja.progeningreflcj. Agencia : '||j.locadepa||'-'||j.locacodi||' grupo '||nugrupo||' concepto '||i.concepto||' '||SQLERRM);
   END;
   END LOOP;
  END LOOP;
  ...
  DELETE valogrup WHERE VAGRanoe = nuanoeval AND VAGRmese =  numeseval;

  FOR i IN (SELECT trfcdepa depa,trfcloca loca,trfcgrup grup,SUM(trfcps1m)+SUM(trfcps2m)+SUM(trfcps3m)+SUM(trfcps4m) suma,SUM(trfcrvca) care,SUM(trfcrvtm) tm FROM tereflcj WHERE trfcanoe = nuanoeval AND trfcmese = numeseval AND trfcfltr = 'I' AND trfctipo IN('C','I') AND trfcfunc = nufunc GROUP BY trfcdepa,trfcloca,trfcgrup) LOOP
   BEGIN
    ...
    UPDATE valogrup
     ...
     WHERE VAGRanoe = nuanoeval
       ...
       AND vagrfunc = nufunc;
    IF SQL%NOTFOUND THEN
       INSERT INTO VALOGRUP(VAGRanoe,VAGRmese,VAGRGRUP,VAGRVALO,vagrfunc) VALUES(nuanoeval,numeseval,i.grup,abs(nuvalora),nufunc);
    END IF;
  ...
  END LOOP;
  FOR i IN (SELECT trfcgrup grup,SUM(trfcps1m) sem1,SUM(trfcps2m) sem2,SUM(trfcps3m) sem3,SUM(trfcps4m) sem4,SUM(trfcrvca) val_ca,SUM(trfcrvtm) tm FROM tereflcj WHERE trfcanoe = nuanoeval AND trfcmese = numeseval AND trfcfltr = 'I' AND trfctipo IN ('C','I') AND trfcfunc = nufunc GROUP BY trfcgrup) LOOP
   BEGIN
    /*SELECT NVL(SUM(trfcrvtm),0) INTO nuvalora
      FROM tereflcj
...
pdiaspago          NUMBER;
pdiasfact          NUMBER;
nuGrupo            flcjtipmov.fctmcdgo%TYPE;
...
CURSOR cudatos(fefi date) IS
  ...
  from moviflcj, factcote, nitero, tipmovim,flcjtipmov,tipmovflcja
 ...
 union
   select distinct mvfctmpr timo,fctmcdgo grupo,fctmineg ineg,mvfcdist distc,mvfccodi codic,mvfcfeea fecha,NVL(SUM(DECODE(TIMVFCEG,'S',mvfcvnet - nvl(mvfcvapl,0) - NVL(MVFCVAPP,0),'s',mvfcvnet - nvl(mvfcvapl,0) - NVL(MVFCVAPP,0),'n',(-1)*(mvfcvnet - nvl(mvfcvapl,0) - NVL(MVFCVAPP,0)),'N',(-1)*(mvfcvnet - nvl(mvfcvapl,0) - NVL(MVFCVAPP,0)))),0) VALOR
     from moviflcj,regppaga, nitero, tipmovim,flcjtipmov,tipmovflcja
   where mvfcesta in (1,2,3,8,9,10) and
  ...
  union
       select distinct mvfctmpr timo,fctmcdgo grupo,fctmineg ineg,mvfcdist distc,mvfccodi codic,mvfcfeea fecha,NVL(SUM(DECODE(TIMVFCEG,'S',mvfcvnet - nvl(mvfcvapl,0) - NVL(MVFCVAPP,0),'s',mvfcvnet - nvl(mvfcvapl,0) - NVL(MVFCVAPP,0),'n',(-1)*(mvfcvnet - nvl(mvfcvapl,0) - NVL(MVFCVAPP,0)),'N',(-1)*(mvfcvnet - nvl(mvfcvapl,0) - NVL(MVFCVAPP,0)))),0) VALOR
              from moviflcj,nitero, tipmovim,flcjtipmov,tipmovflcja
              where mvfcesta in (1,2,3,8,9,10) and
  ...
  /*SELECT fctmineg ineg,mvfcdist distc,mvfccodi codic,FCTMCDGO grupo,mvcbesta esta,mvcbtimv timo,mvcbdist dist,mvcbcons cons,mvcbfeapl fecha,mvcbvlor valor,mvcbdisa disa,mvcbtmad tmad,mvcbcasi casi
   FROM  movicuba,mvfcmvcb,moviflcj,flcjtipmov,tipmovflcja
...
-- Se borran los datos del a?o y mes a evaluar
 ...
 FOR i IN cudatos(dtfechfins4) LOOP
   IF i.fecha <= dtfechfins1 THEN
    ...
    END IF;
 END LOOP;
 FOR i IN (SELECT grupo,NVL(SUM(sem1),0) sem1,NVL(SUM(sem2),0) sem2,NVL(SUM(sem3),0) sem3,NVL(SUM(sem4),0) sem4 FROM egreflcj WHERE ano = nuanoeval AND mes = numeseval AND tipo = 'E' AND func = nufunc GROUP BY grupo) LOOP
   BEGIN
    SELECT NVL(SUM(sem1),0),NVL(SUM(sem2),0),NVL(SUM(sem3),0),NVL(SUM(sem4),0) INTO nusem1,nusem2,nusem3,nusem4
      FROM egreflcj
     WHERE ano   = nuanoeval
       AND mes   = numeseval
       AND grupo = i.grupo
 ...
 FOR i IN cuegrereales(dtfechinic,dtfechfins4) LOOP
    ...
    BEGIN
       INSERT INTO VCCBFLCJ(distb,codib) VALUES(i.dist,i.cons);
     EXCEPTION
      WHEN DUP_VAL_ON_INDEX THEN
     ...
     END;
 END LOOP;
 FOR i IN (SELECT grupo,NVL(SUM(vare),0) vare FROM egreflcj WHERE ano = nuanoeval AND mes = numeseval AND tipo = 'E' AND func = nufunc GROUP BY grupo) LOOP
  BEGIN
 ...
 EXCEPTION
  WHEN OTHERS THEN
   RAISE_APPLICATION_ERROR(-20024,'Error en PKFLUJOCAJA.progeneegresoflcj '||sqlerrm);
END progeneegresoflcj;

PROCEDURE proprocesar1 IS
BEGIN
    pkflujocaja.ProProyIngFactFlujoCaja;
-- pkflujocaja.ProProyIngFactFlujoCaja@lin_sucre;
-- pkflujocaja.ProProyIngFactFlujoCaja@lin_cordoba;
END proprocesar1;

PROCEDURE proprocesar2 IS
BEGIN
    pkflujocaja.proypagosflucaja;
-- pkflujocaja.proypagosflucaja@lin_sucre;
-- pkflujocaja.proypagosflucaja@lin_cordoba;
END proprocesar2;

PROCEDURE proprocesar3 IS
BEGIN
    pkflujocaja.proconsvalrealingcarg;
-- pkflujocaja.proconsvalrealingcarg@lin_sucre;
-- pkflujocaja.proconsvalrealingcarg@lin_cordoba;
END proprocesar3;
...
END proInicial;
    ...
    pkflujocaja.numesrfc := numese;
    pkflujocaja.nuanorfc := nuanoe;
