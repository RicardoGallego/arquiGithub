...
BEGIN
  ...
  dtfechdia := dtfechinic;
     ...
     FOR i IN 1..to_char(dtfechfini, 'DD') LOOP 
        ...
        SELECT dtfechinic + i INTO dtfechdia
          FROM dual;
        
        INSERT INTO peevflcj_sap VALUES(anoeval,meseval,'P',nufunc, dtfechdia);
        --dbms_output.put_line(fechaDia||' : '|| to_char(last_day(fechaInic), 'DD'));
    END LOOP;
...
--     ano1:=anoeval;
   ...
   -->>
    dtfechinic := TO_DATE('01/'||fsbmesletras(meseval)||'/'||anoeval,'DD/MON/YYYY');
    SELECT last_day(dtfechinic) INTO dtfechfini FROM dual;
    dtfechdia := dtfechinic;
    ...
    INSERT INTO compreca_sap(
                ...
                VALUES(
                      ...
                      ano1,mes2,ano2,sumvafa,sumrec1,sumrec2,nufunc,dtfechdia
                     );  
...
BEGIN
  ...
  --averiguar recaudo por semana en mes1
      ...
      dtfechdia := dtFechInic1;
      FOR i IN 0..to_char(dtFechF1, 'DD') LOOP 
         SELECT nvl(sum(nvl(MVCBVLOR,0)),0) INTO recames1dia
          FROM movicuba_sap
         WHERE mvcbtimv in (313,334,1069,994)
           AND mvcbfeapl >=dtFechInic1+i
           AND mvcbfeapl <=dtFechInic1+i+1;
         ...
         INSERT INTO recaproy_sap(
                     ...
                     VALUES(
                            anoeval,meseval,mes1,ano1,nufunc,porcdiames1, dtFechInic1+i
                           );
         
        INSERT INTO peevflcj_sap VALUES(anoeval,meseval,'R',nufunc, dtFechInic1+i);
 ...
 *************************************************************************/
    ...
    -- Consultamos el valor de los cargos
    -- Se extrae un campo de mas en la consulta
    
    CURSOR cuCargreal IS
    SELECT depa,loca,ano,mes,grupo,concep,nvl(SUM(valor),0) valor, dia
      FROM inreflcj_sap 
     WHERE ano = nuanoe AND mes = numese AND func = nufunc AND dia = dtdiae
 ...
 --<<
 -- Se le agrego el campo trfcdiae a la tabla tereflcj_sap
 -->>
   ...
   UPDATE tereflcj_sap
    ...
    IF sql%notfound THEN
     ...
     INSERT INTO tereflcj_sap(trfcdepa,trfcloca,trfcanoe,trfcmese,trfcgrup,trfcconc,trfcfltr,trfcrvca,trfctipo,trfcfunc, trfcdiae)
                  VALUES(i.depa,i.loca,i.ano,i.mes,i.grupo,i.concep,'I',i.valor,'C',nufunc, dtdiae );
    END IF;
  END LOOP;
...
DELETE ctabflcj_sap WHERE ano = nuanorfc AND mes = numesrfc AND func = nufunc;
   ...
   SELECT last_day(dtfechaini) INTO dtfechafin
     FROM dual;
   dtfechafin  := to_char(to_date(dtfechafin,'DD/MM/YY')-27, 'DD/MM/YY');
 ...
 *************************************************************************/
  ...
  -->>  
  CURSOR cuCargreal IS
    SELECT depa,loca,ano,mes,grupo,tipmov,tipo,nvl(sum(valor),0) nuvalor, dia
      FROM ctabflcj_sap
     WHERE ano = nuanoe
       AND mes = numese
