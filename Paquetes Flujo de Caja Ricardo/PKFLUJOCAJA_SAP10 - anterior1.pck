...
PROCEDURE procrearegfactconc(nudepa NUMBER,nuloca NUMBER,nuanoe NUMBER,numese NUMBER,nuano NUMBER,
                             numes NUMBER,nuconc NUMBER,nuValor NUMBER,nugrupo NUMBER, nuDia NUMBER);
...
PROCEDURE procrearegfactconc(nudepa NUMBER,nuloca NUMBER,nuanoe NUMBER,numese NUMBER,
                             ...
                             nugrupo NUMBER, nuDia NUMBER) IS
/***********************************************************************************************
...
CURSOR cuCargos(dtFechI DATE,dtFechF DATE) IS
    SELECT /*+ index (cargos idxcargos11)*/ locadist cargdepa,locaagen cargloca,cargconc,
     ...
     WHERE cargfecr >= dtFechI
      ...
      GROUP BY locadist,locaagen,cargconc,cargcate, to_char(cargfecr, 'DD')
    UNION ALL
    SELECT /*+ index (cargos idxcargos11)*/  locadist cargdepa,locaagen cargloca,cargconc,
           sum(decode(cargsign,'DB',nvl(cargvalo,0),nvl(-cargvalo,0))) SUMA,cargcate cate,
           to_char(cargfecr, 'DD')  dia 
...
BEGIN
   ...
   --<<
   -- Ricardo Se modificaron los formatos de fehca de (MM/DD/YYYY) a (MM/DD/YY)
 ...
 *************************************************************************/
  ...
  CURSOR cuCargreal IS
    SELECT depa,loca,ano,mes,grupo,concep,nvl(SUM(valor),0) valor FROM inreflcj_sap WHERE ano = nuanoe AND mes = numese AND func = nufunc group by depa,loca,ano,mes,grupo,concep;
 --  UNION ALL
