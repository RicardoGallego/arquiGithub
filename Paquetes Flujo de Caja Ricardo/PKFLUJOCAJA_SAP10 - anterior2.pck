...
***********************************************************************************************/
    ...
    CURSOR cuco(nuanocu NUMBER,numescu NUMBER, dtdiafech DATE) IS
        SELECT cucoano,cucomes,cucocodi,cucovato,cucovaab/*,
               to_char(cucofepa, 'DD/MM/YYYY') dia */   
         ...
         WHERE cucoprog = 'FGCC'
           ...
           --
           -->>
           --AND to_char(cucofege, 'DD/MM/YYYY') = to_char(dtdiafech, 'DD/MM/YYYY') 
/*      GROUP BY cucoano,cucomes,cucocodi,cucovato,cucovaab,
               to_char(cucofepa, 'DD/MM/YYYY');*/


    --<<
    -- Se adiciona un campo mas para extraer de la consulta
    -- este campo es el dia cargfecr (fecha creacion)
    -->>
...
--     FOR i IN 1..to_char(dtfechfini, 'DD') LOOP 
       ...
       -- Calculamos el facturado del mes evaluado
         FOR c1 IN cuco(nuano1,meseval,dtdiafech) LOOP
         ...
         INSERT INTO compreca_sap(
                     ...
                     VALUES(
                           ...
                           ano2,sumvafa,sumrec1,sumrec2,nufunc,dtdiafech
                          );  
                          
          INSERT INTO peevflcj_sap VALUES(anoeval,meseval,'P',nufunc, dtdiafech);  
          
          -- Se hace incrementa la fecha aca, para que me tome las fechas adecuadas     
--          SELECT dtfechinic + i INTO dtdiafech
--            FROM dual;
    ...
    -->>
     ...
     FOR i IN 1..to_char(last_day(dtFechInic1), 'DD') LOOP 
         SELECT nvl(sum(nvl(MVCBVLOR,0)),0) INTO recames1dia
           FROM movicuba
          WHERE mvcbtimv in (313,334,1069,994)
            AND to_char(mvcbfeapl, 'DD/MM/YYYY') = dtdiafech;
          
          porcdiames1:=round( (recames1dia/recames1)*100,4);
        
          --<<
          -- modificar la tabla recaproy_sap
          -- agregar el campo reprdiae
          -- agregar el campo reprdiapm
          -- eliminar el campo reprs1m1
          -- eliminar el campo reprs2m1
          -- eliminar el campo reprs3m1
          -- eliminar el campo reprs4m1
          -->>
 ...
 -- Generamos los ingresos
   ...
   -- Valor Facturado 1mes
       ...
       -- Generamos los ingresos proyectados por dia
             ...
             FOR cont IN 1..to_char(dtfechinic, 'DD') LOOP 
    ...
    --
            ...
            -- Se graban los datos en la tabla
                 ...
                 SELECT dtfechinic + cont INTO dtdiafech FROM dual;
...
--
    ...
    FOR cont IN 1..to_char(dtfechinic, 'DD') LOOP 
        ...
        pkflujocaja_sap10.procarginreal(nuanoeval,numeseval,dtdiaeval);
        pkflujocaja_sap10.procarginrealtipmov(nuanoeval,numeseval,dtdiaeval);

        DELETE valogrup_sap  WHERE VAGRanoe = nuanoeval AND VAGRmese =  numeseval 
               AND to_char(VAGRdiae, 'DD/MM/YYYY') = to_char(dtdiaeval, 'DD/MM/YYYY');
        ...
        FOR i IN (SELECT trfcdepa depa,trfcloca loca,trfcgrup grup,
                   ...
                   WHERE trfcanoe = nuanoeval 
                    ...
                    -->>
                     ...
                     AND trfctipo IN('C','I') 
                     AND trfcfunc = nufunc 
                   GROUP BY trfcdepa,trfcloca,trfcgrup, trfcdiae) LOOP
            BEGIN
             ...
             WHERE VAGRanoe = nuanoeval
               ...
               IF SQL%NOTFOUND THEN
                   INSERT INTO valogrup_sap(VAGRanoe,VAGRmese,VAGRGRUP,VAGRVALO,vagrfunc, vagrdiae) 
                          VALUES(nuanoeval,numeseval,i.grup,abs(nuvalora),nufunc, dtdiaeval);
               END IF;
